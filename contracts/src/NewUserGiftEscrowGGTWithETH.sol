...
/**
 * @title NewUserGiftEscrowGGTWithETH
 * @dev Smart contract for creating crypto gifts for new users with GGT tokens AND ETH for gas
 * Uses claim codes instead of wallet addresses for recipients
 * Supports multiple unlock mechanisms: simple, password, quiz, GPS
 * Includes ETH to solve the gas fee problem for new users
 */
contract NewUserGiftEscrowGGTWithETH is Ownable, ReentrancyGuard, Pausable {
    ...
    struct NewUserGift {
        ...
        uint256 ethAmount;          // Amount of ETH for gas fees
    ...
    /**
     * @dev Create a new user gift with claim code using GGT tokens and ETH
     * @param claimHash Hash of the claim code (keccak256 of claim code)
     * @param ggtAmount Amount of GGT tokens to gift
     * @param ethAmount Amount of ETH to include for gas fees
    ...
    function createNewUserGift(
        ...
        require(msg.value == ethAmount, "ETH sent must match ethAmount");
        require(ethAmount >= 0.001 ether, "ETH amount must be at least 0.001");
        ...
        GGT_TOKEN.safeTransferFrom(msg.sender, address(this), ggtAmount);
        ...
        emit NewUserGiftCreated(giftId, msg.sender, ggtAmount, ethAmount, expiry, unlockType);
    }
    ...
    function claimGift(
        ...
        GGT_TOKEN.safeTransfer(msg.sender, gift.ggtAmount);
        
        // Transfer ETH to recipient for gas fees
        if (gift.ethAmount > 0) {
            (bool success, ) = payable(msg.sender).call{value: gift.ethAmount}("");
            require(success, "ETH transfer failed");
        }
        
        emit GiftClaimed(giftId, msg.sender, gift.ggtAmount, gift.ethAmount);
    }
    ...
    function refundExpiredGift(bytes32 giftId) external nonReentrant {
        ...
        if (gift.ethAmount > 0) {
            (bool success, ) = payable(gift.sender).call{value: gift.ethAmount}("");
            require(success, "ETH refund failed");
        }
        
        emit GiftRefunded(giftId, gift.sender, gift.ggtAmount, gift.ethAmount);
    }
     ...
     * @return ggtAmount Amount of GGT tokens
     * @return ethAmount Amount of ETH for gas
    ...
    /**
     * @dev Get GGT token balance of the contract
     * @return The GGT token balance
     */
    function getContractGGTBalance() external view returns (uint256) {
        return GGT_TOKEN.balanceOf(address(this));
    }
     ...
     * @param withdrawETH Whether to withdraw ETH balance
     */
    function emergencyWithdraw(bool withdrawETH) external onlyOwner {
        // Withdraw GGT tokens
